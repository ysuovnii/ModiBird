...
# bird = []
# for i in range(1, 4):
#     frame = pygame.image.load(f'assets/qyellowbird-{i}.png').convert_alpha()
#     bird.append(frame)

# bird_rect = bird[0].get_rect(center = (40, 250))
modi = pygame.image.load(f'./assets/modi.png').convert_alpha()
modi = pygame.transform.scale(modi, (42, 62))
modi_rect = modi.get_rect(center = (40, 250))

pipe = pygame.image.load("./assets/final.png").convert_alpha()
...
resume = pygame.image.load("assets/Resume.png").convert_alpha()
...
while run: 
    ...
    if game_active: 
        ...
        for pipe_rect in pipe_list:
            ...
            if modi_rect.colliderect(pipe_rect):
                game_active = False

        get_score()

        screen.blit(base, base_rect)
        screen.blit(modi, modi_rect)

    else: 
        game_over()
        if event.type == pygame.MOUSEBUTTONDOWN: 
            if resume_rect.collidepoint(mouse_pos):
                modi_rect.center = (40, 250)
                pipe_list.clear()
